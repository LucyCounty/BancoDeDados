...
class controller_Filme:
    ...
    def atualizar_filme(self) -> filmes:
    ...
    @staticmethod
    def get_filme_from_dataframe(oracle:OracleQueries, id_filme:int=None) -> filmes:
        # Recupera os dados do novo filme criado transformando em um DataFrame
        df_filme = oracle.sqlToDataFrame(f"select id_filme, titulo, ano_publicacao, quantidade from filmes where id_filme = {id_filme}")
        # Cria novo objeto a partir do DataFramefilme
        return filmes(df_filme.id_filme.values[0], df_filme.titulo.values[0], df_filme.values[0], df_filme.ano_publicacao.values[0], df_filme.quantidade.values[0])
    
    @staticmethod
    def valida_filme(oracle:OracleQueries, id_filme:int=None) -> filmes:
        if not controller_Filme.verifica_existencia_filme(oracle, id_filme):
    ...
    @staticmethod
    def valida_filme_disponivel(oracle:OracleQueries, id_filme:int=None) -> filmes:
        if not controller_Filme.verifica_existencia_filme(oracle, id_filme):
            print(f"O filme de código {id_filme} não existe na base.")
            return None
